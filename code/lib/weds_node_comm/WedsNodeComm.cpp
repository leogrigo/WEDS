...
WedsNodeComm::WedsNodeComm()
    : node_id_(0), sequence_id_(1), initialized_(false), radio_sleeping_(false),
      has_last_gateway_command_(false), last_gateway_command_sequence_id_(0),
...
bool WedsNodeComm::begin() {
  ...
  has_last_gateway_command_ = rtc_node_comm_state.has_last_gateway_command;
...
void WedsNodeComm::loop() { heltec_loop(); }

bool WedsNodeComm::initRadio() {
  Serial.println("[NODE_COMM] Initializing LoRa radio...");

  int state = radio.begin(WEDS_LORA_FREQUENCY_MHZ, WEDS_LORA_BANDWIDTH_KHZ,
                          WEDS_LORA_SPREADING_FACTOR, WEDS_LORA_CODING_RATE,
                          WEDS_LORA_SYNC_WORD, WEDS_LORA_OUTPUT_POWER_DBM,
...
bool WedsNodeComm::sendStatus(const WedsNodeStatusPayload &status) {
  ...
  bool ok = weds_build_node_status_packet(node_id_, status_sequence_id, status,
                                          packet);
...
bool WedsNodeComm::sendAlert(const WedsNodeStatusPayload &status) {
  ...
  bool ok =
      weds_build_node_alert_packet(node_id_, alert_sequence_id, status, packet);
  ...
  for (uint8_t attempt = 1; attempt <= WEDS_NODE_ALERT_MAX_RETRIES; ++attempt) {
    ...
    if (!tx_ok) {
      ...
      bool ack_ok = waitForAck(alert_sequence_id, WEDS_MSG_NODE_ALERT,
...
bool WedsNodeComm::sendPacket(const WedsPacket &packet) {
  ...
  bool ok = weds_serialize_packet(packet, buffer, sizeof(buffer), encoded_len);
...
bool WedsNodeComm::waitForAck(uint16_t expected_acked_sequence_id,
  ...
  while (millis() - start_ms < timeout_ms) {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      ...
      bool ok = weds_deserialize_packet(rx_buffer, len, packet);
      ...
      if (isExpectedAck(packet, expected_acked_sequence_id,
...
bool WedsNodeComm::isExpectedAck(const WedsPacket &packet,
  ...
  bool ok =
      weds_decode_ack_payload(packet.payload, packet.header.payload_len, ack);
...
uint32_t WedsNodeComm::getNodeId() const { return node_id_; }

uint16_t WedsNodeComm::getCurrentSequenceId() const { return sequence_id_; }

void WedsNodeComm::persistRtcState() const {
  rtc_node_comm_state.magic = WEDS_NODE_COMM_RTC_MAGIC;
  rtc_node_comm_state.version = WEDS_NODE_COMM_RTC_VERSION;
  rtc_node_comm_state.sequence_id = sequence_id_ == 0 ? 1 : sequence_id_;
  rtc_node_comm_state.has_last_gateway_command = has_last_gateway_command_;
  rtc_node_comm_state.last_gateway_command_sequence_id =
      last_gateway_command_sequence_id_;
  rtc_node_comm_state.last_gateway_command_msg_type =
      last_gateway_command_msg_type_;
...
bool WedsNodeComm::pollAlertModeEnable(WedsAlertModeEnablePayload &out_command,
                                       uint32_t timeout_ms) {
  if (!initialized_) {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      ...
      if (isDuplicateGatewayCommand(packet)) {
        ...
        if (packet.header.ack_required == WEDS_ACK_REQUIRED) {
          sendAck(WEDS_GATEWAY_ID, packet.header.sequence_id,
                  packet.header.msg_type, WEDS_ACK_STATUS_OK);
        }
      ...
      bool payload_ok = weds_decode_alert_mode_enable_payload(
          packet.payload, packet.header.payload_len, out_command);

      if (!payload_ok) {
        Serial.println(
            "[NODE_COMM] Failed to decode ALERT_MODE_ENABLE payload");

        if (packet.header.ack_required == WEDS_ACK_REQUIRED) {
          sendAck(WEDS_GATEWAY_ID, packet.header.sequence_id,
                  packet.header.msg_type, WEDS_ACK_STATUS_INVALID_PACKET);
        }
...
bool WedsNodeComm::sendAck(uint32_t dst_node_id, uint16_t acked_sequence_id,
                           uint8_t acked_msg_type, uint8_t status_code) {
  WedsPacket ack_packet;

  const uint16_t ack_sequence_id = sequence_id_++;
  persistRtcState();

  bool ok = weds_build_ack_packet(node_id_, dst_node_id, ack_sequence_id,
                                  acked_sequence_id, acked_msg_type,
                                  status_code, ack_packet);
