...
void printSample(const WedsSensorSample &sample) {
  Serial.printf("[SENSOR] temp=%.2f C hum=%.2f %% pressure=%.2f Pa "
                "gas_raw=%.0f gas=%.1f %%\n",
...
/**
 * @brief Funzione fittizia per retrocompatibilità. Senza BSEC non ci sono stati
 * d'errore complessi.
 */
bool checkBmeSensorStatus(void) { return true; }
...
bool weds_sensors_begin() {
  ...
  bmeSensor.setGasHeater(320, 1000);
...
WedsSensorSample weds_read_environment_sample() {
  ...
  // bmeSensor.performReading() "sveglia" il sensore, scalda il gas, preleva i
  // dati e lo spegne
...
int64_t weds_sensor_next_call_ms() {
  // Con Adafruit decidi tu quando chiamare il sensore (es. ogni 10 minuti in
  // deep sleep). Restituiamo 0 in modo da informare il sistema che non ci sono
  // vincoli di attesa.
  return 0;
}
...
void weds_print_sample(const WedsSensorSample &sample) {
  Serial.printf("[SENSOR] timestamp=%lu temp=%.2f C hum=%.2f %% pressure=%.2f "
                "Pa gas=%.2f valid=%s\n",
                (unsigned long)sample.timestamp, sample.temperature,
                sample.humidity, sample.pressure, sample.gas_resistance,
                sample.valid ? "yes" : "no");
}