...
constexpr float WEDS_ANOMALY_MIN_GAS_STDDEV_PERCENTAGE = 0.1f;
...
WedsAnomalyDetector::WedsAnomalyDetector()
    : started_(false), samples_seen_(0), last_result_{} {
...
WedsAnomalyResult WedsAnomalyDetector::update(const WedsSensorSample &sample) {
  ...
  const float min_stddev =
      prev_gas_baseline * WEDS_ANOMALY_MIN_GAS_STDDEV_PERCENTAGE;
  const float gas_z = gas_residual / max(prev_gas_stddev, min_stddev);
  ...
  if (result.detection_state == WEDS_DETECTION_NORMAL || warming_up) {
    result.gas_baseline = gas_filter_.update(sample.gas_resistance);
    result.gas_stddev =
        sqrtf(gas_variance_filter_.update(gas_residual * gas_residual));
    if (result.gas_stddev < min_stddev) {
      result.gas_stddev = min_stddev;
    }
  } else {
...
uint32_t WedsAnomalyDetector::samplesSeen() const { return samples_seen_; }
...
void printAnomalyResults(const WedsAnomalyResult &result) {
  Serial.printf("[ANOMALY] state=%s score=%.2f baseline=%.0f stddev=%.1f "
                "samples=%lu warmup=%s\n",
                result.detection_state == WEDS_DETECTION_ALERT ? "ALERT"
                                                               : "NORMAL",
                result.gas_score, result.gas_baseline, result.gas_stddev,
